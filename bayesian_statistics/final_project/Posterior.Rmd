...
stanfit = stan(
    "bym2.stan",
    data=list(N, N_edges, node1, node2, K, x, y),
...
saveRDS(stanfit, file = "stanfit_object.rds")
```
...
map$log_median_value <- log(map$MEDV)
y = map$log_median_value
x = cbind(map$RM, log(map$DIS), map$NOX)
K = dim(x)[2]
stanfit = stan(
    "bym2.stan",
    data=list(N, N_edges, node1, node2, K, x, y),
...
check_hmc_diagnostics(stanfit)
print(stanfit, probs=c(0.25, 0.75), digits_summary=1)
saveRDS(stanfit, file = "stanfit_object_crim.rds")
```
